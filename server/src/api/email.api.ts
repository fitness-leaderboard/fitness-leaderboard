...
export const sendVerificationEmail = async (
  ...
  try {
    ...
    if (Token.length !== 6 || !Token.match(/^[0-9A-Z]+$/)) {
      return res
        .status(400)
        .json({ error: 'Invalid token provided. Must be six characters and contain only 0-9A-Z' });
    }

    const verifyEmailHtml = fs
      .readFileSync(path.join(__dirname, '../../public/verifyEmail.html'), 'utf8')
      .replace('verifyTokenPlaceholder', Token);
...
export const sendForgotPasswordEmail = async (
  ...
  const forgotPasswordEmailHtml = fs.readFileSync(
